import {Badge, Box, Center, Heading, IconButton, Spinner, Table, TableCaption, TableContainer, Tbody, Td, Th, Thead, Tr} from "@chakra-ui/react"
  ...
  return (
            <TableContainer>
                      ...
                      <Table variant='simple' size={{ base: "sm", md: "md" }} >
                          ...
                          <Tbody>
                              {props.isLoading?<Center><Spinner/></Center>:props.controlPlanes.map((item: any) => {
                                      return <Tr key={item.id}>
                                          <Td fontWeight={"medium"}>{item.id}</Td>
                                          <Td fontWeight={"bold"}>{item.name}</Td>
                                          <Td >{item.region}</Td>
                                          <Td> <Badge colorScheme={item.status === 'ACTIVE' || item.status === 'CREATED'  ? 'green' : 'red'}>{item.status}</Badge></Td>
                                          <Td fontWeight={"light"}>{item.createdAt}</Td>
                                          <Td>
                                              <IconButton icon={<ViewIcon fontSize={"2xl"} />} colorScheme={'green'} aria-label='View' border={"none"} variant="link" onClick={() => props.onView(item.id)}/>
                                               <IconButton icon={<EditIcon fontSize={"2xl"} />} colorScheme={'orange'} aria-label='View' border={"none"} variant="link"/>
