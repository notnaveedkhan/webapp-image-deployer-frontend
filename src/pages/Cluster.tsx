...
export default function Cluster() {
    ...
    const handleViewControlPlane = (controlPlaneId: string) => { 
        navigater(`/cluster/control-plane/${controlPlaneId}`);
    }
    
  return (
      <Box>
          <Box mt={5} p={6}>
              <ControlPlaneTable isLoading={controlPlaneDataLoading} controlPlanes={controlPlane} onRefresh={handelRefetchControlPlane} onView={handleViewControlPlane} />
              <NodeGroupTables isLoading={nodeGroupDataLoading} nodeGroup={nodeGroup} controlPlane={controlPlane} onRefresh={handelRefetchNodeGroup} />
          </Box>
      </Box>
  )
}
