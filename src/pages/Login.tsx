...
import { Box, Button, Divider, FormControl, FormErrorMessage, FormLabel, Image, Input, InputGroup, InputLeftElement, InputRightElement, Link, Text } from "@chakra-ui/react";
import { useState } from "react";
import KubernativeLogo from '../assets/Kubernetes_logo_without_workmark.svg.png';
...
export default function Login() {
  ...
  return (
      <Box bgGradient='linear(to-b,blue,blue.200)' w={"100%"} h={"100vh"} display={"flex"}  flexDir="column" gap={4} justifyContent="center" alignItems={"center"}>
          <Image src={KubernativeLogo} alt="LOGO" w={"100px"} />
          <Box p={4} borderRadius="2xl" bgColor={"white"} w={{"base":"100%" , "md":"30%" , "lg":"30%"}} >
              ...
              <FormControl p={3}>
                  ...
                  <InputGroup>
                      ...
                      <Input name="email" id="email"    onChange={handleChange} type={"email"} placeholder="Email Address" />
                      </InputGroup>
              ...
              <FormControl p={3}>
                  ...
                  <InputGroup>
                      ...
                      <Input name="password" id="password" onChange={handleChange} type={show?"text":"password"} placeholder="Password" />
              ...
              <Box display={"flex"} gap={2} justifyContent="center"  paddingY={3} bgColor={"gray.100"}>
                  <Text fontSize={"md"}>New Account ?</Text>
                  <Link fontSize={"md"} color={"blue.500"}>SignUp</Link>
              </Box>
          </Box>
          <Box >
              <Link color={"white"} >Forget Password</Link>
          </Box>
    </Box>
  )
}
